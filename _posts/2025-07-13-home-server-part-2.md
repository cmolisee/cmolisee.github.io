...
Enable the PCIe port by adding `dtparam=pciex1` to the end of `/boot/firmware/config.txt` file and reboot. After the reboot you can use `lsblk` to check the disk.

**Step 3 (Optional):** You can follow the guide mentioned above and do a speed test before and after enabling Gen 3 for the PCIe port. I think its worth the effort to at least enable Gen 3 but it is not necessary. To do that you just append `dtparam=pciex1_gen=3` in the same file as before `/boot/firmware/config.txt`.

_BONUS: At the end of the documentation linked above you can find some cool schematics for DIY 3D printed cases. It is a little too much effort for me so i'll just leave as is or use a ceral box or something._
